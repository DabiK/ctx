...
export function buildPrompt(): string {
  return [
    ...
    `- \`batch\` — compose supported reads into one response`,
    ``,
    `# Proposing changes`,
    ...
    `A proposed patch or write is surfaced for review; it is applied only through the`,
    `approved ${PRODUCT_NAME} flow.`,
    ``,
    `# Responses`,
