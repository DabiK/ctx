  ...
  switch (command) {
    ...
    case "watch":
      return [
        ...
        `c cancel pending write, p preview pending write, i copy the startup protocol`,
        `(like ${EXECUTABLE_NAME} prompt: the protocol plus the root AGENTS.md, without`,
        `creating or overwriting .ctx.toml/.ctxignore), q quit.`,
        ``,
        ...
        `structured response.`,
        ``,
        `The interface renders through OpenTUI, which requires Bun 1.3+ or`,
        `Node.js 26.4.0 (run with --experimental-ffi). On other runtimes the`,
        `watcher refuses to start with the exact command to run.`,
        ``,
        `Options:`,
